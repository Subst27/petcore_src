...
#RESOURCES += res/styles/dark/darkstyle.qrc
#RESOURCES += res/styles/light/lightstyle.qrc
OTHER_FILES +=
...
# LimeReport
LR_DIR=$$(lr_install_path) # из переменных среды взять
!isEmpty(LR_DIR) {
  LIBS += -L$$LR_DIR -llimereport -lQtZint
  INCLUDEPATH += $$LR_DIR/include
  }

isEmpty(LR_DIR) {
  win32 | android: LR_DIR = $$[QT_INSTALL_PREFIX]/3rdparty/LimeReport
  unix: !android: LR_DIR = $$PWD/../LimeReport/install
