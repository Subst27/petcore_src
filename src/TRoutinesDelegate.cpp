...
TRoutinesDelegate::TRoutinesDelegate(QObject *parent) : QStyledItemDelegate{parent}
  {
  /* перерыв на обед - TPalette::Unavailebal
   * суббота         - TPalette::Saturday
...
/*QSize TRoutinesDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
  {
  Q_UNUSED(index)
