...
TMainWnd::TMainWnd(QWidget *parent) :
  ...
  m_remindersDelegate=new TRemindersDelegate(this);
  remindersTable->setItemDelegate(m_remindersDelegate);
  ...
  if (m_remindersModel->rowCount()>0)
    remindersTable->selectRow(0);
  ...
  // ToolBar для основных данных (на странице DataPage: Визиты, Питомцы, Клиенты, Доктора)
...
void TMainWnd::writeSettings()
  ...
  // сохранить хедер расписания
  settings.setXmlValue("main_window/data_view/shedule","",sheduleTree->header()->saveState());
  ...
  settings.setXmlValue("tasks_dock/remider_table","",remindersTable->horizontalHeader()->saveState());

  QString mattersName;
  if (qApp->arguments().contains("-develop")==true)
    mattersName=qApp->applicationDirPath()+"/matters.txt";
  else
    mattersName=QStandardPaths::writableLocation(QStandardPaths::AppDataLocation)+"/matters.txt";

  QFile mettersFile(mattersName);
  if (mettersFile.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text))
    mettersFile.write(mattersEdit->toPlainText().toUtf8());
  }
...
void TMainWnd::readSettings()
  ...
  // расписание
  sheduleTree->header()->restoreState(settings.getXmlValue("main_window/data_view/shedule","",QByteArray()).toByteArray());
  ...
  remindersTable->horizontalHeader()->restoreState(settings.getXmlValue("tasks_dock/remider_table","",QByteArray()).toByteArray());
  ...
  m_remindersDelegate->setReminderDeadline(settings.getXmlValue("tasks_dock/reminder_deadline","",3).toUInt());
  ...
  QFile mettersFile(mattersName);
  if (mettersFile.open(QIODevice::ReadOnly | QIODevice::Text))
    mattersEdit->setPlainText(mettersFile.readAll());
  }
...
void TMainWnd::createSplashScreen(const QString &text)
  ...
  QPixmap pixmap(":/images/petcore_640_360");
  int fontSize=20;
  ...
  // надо разбить текст на строки, которые будут умещаться по ширине, этакий WordWrap
...
void TMainWnd::pageTabChanged(int index)
  {
  TMainWnd::StackedPages lastActivePage=(TMainWnd::StackedPages)pagesStacked->currentIndex();

  // запомнить выделенную строку до смены модельки у таблицы
  QItemSelectionModel *selectionModel=m_lastActiveTab==TMainWnd::SheduleTab ? sheduleTree->selectionModel() : dataTable->selectionModel();
  ...
  if (lastActivePage==TMainWnd::DataPage)
    {
  ...
  switch ((TMainWnd::PageTabs)index)
    ...
    case TMainWnd::PetsTab:
      ...
      actionIcons=QStringList{":/images/append_pet",":/images/edit_pet",":/images/remove_pet"};
    ...
    case TMainWnd::ClientsTab:
      ...
      actionIcons=QStringList{":/images/append_client",":/images/edit_client",":/images/remove_client"};
    ...
    case TMainWnd::DoctorsTab:
      ...
      actionIcons=QStringList{":/images/append_doctor",":/images/edit_doctor",":/images/remove_doctor"};
  ...
  for (quint8 i=0;i<actions.size();i++)
    {
    actions[i]->setIcon(i<actionIcons.size() ? QIcon(actionIcons.at(i)) : QIcon());
...
void TMainWnd::sheduleTreeActivated(const QModelIndex &index)
  ...
  QModelIndex current=m_sheduleModel->index(index.row(),TSheduleModel::Title,index.parent());
  if (sheduleTree->isExpanded(current))
    {
    sheduleTree->collapse(current);
    return;
    }
  ...
  switch (type)
    ...
    case TSheduleModel::DateType: // ткнули в Дату
      {
      buildDateBranch(current);
      break;
      }
    case TSheduleModel::DoctorType: // ткнули в Доктора
      {
      buildDoctorBranch(current);
      break;
      }
    case TSheduleModel::TimeType: // ткнули в Запись о приеме
      ...
      if (apptId==0)
        {
        if (appendApptActn->isEnabled()==true)
          appendAppt();
        }
      else
        {
        if (editApptActn->isEnabled()==true)
          editAppt();
        }
...
void TMainWnd::buildDateBranch(const QModelIndex &dateIndex)
  ...
  qApp->restoreOverrideCursor();
  if (m_sheduleModel->rowCount(dateIndex)>0)
...
void TMainWnd::buildDoctorBranch(const QModelIndex &doctorIndex)
  ...
  qApp->restoreOverrideCursor();
  if (m_sheduleModel->rowCount(doctorIndex)>0)
...
void TMainWnd::expandShedule(const QMap<QDate, QList<quint32> > &expanded)
  {
  if (m_dataChanged==true)
    {
    m_apptsModel->select();
...
void TMainWnd::showSheduleAppts(QMap<QDate, QList<quint32>> expanded)
  ...
  m_pagesTabBar->setCurrentIndex(TMainWnd::SheduleTab);
  }
...
void TMainWnd::renewTimeSlots()
  ...
  if (pageTab==TMainWnd::SheduleTab && slotType!=TMainWnd::AllTimeSlots)
    {
    QMap<QDate, QList<quint32>> expanded=solveExpanded();
  ...
  for (qint32 i=apptsFilterModel.rowCount()-1;i>-1;i--)
    ...
    bool visible=(pageTab==TMainWnd::SheduleTab && sheduleTree->isExpanded(doctorIndex)==true && timeIndex.isValid()==true);
  ...
  if (pageTab==TMainWnd::SheduleTab && dataChanged==true)
    {
    sheduleTree->hide();
    sheduleTree->show();
    }
...
void TMainWnd::appendAppt()
  ...
  TApptDlg apptDlg(TMainWnd::SheduleTab,apptRecord,this);
...
void TMainWnd::sendTelegram()
  ...
  if ((TMainWnd::PageTabs)m_pagesTabBar->currentIndex()==TMainWnd::SheduleTab)
    {
    QModelIndexList indexes=sheduleTree->selectionModel()->selectedRows();
    if (indexes.size()==0)
      return;
...
void TMainWnd::appendReminder()
  ...
  TReminderDlg reminderDlg(clientName,this);
  if (reminderDlg.exec()!=QDialog::Accepted)
    return;

  QSqlRecord record=reminderDlg.record();
...
TMainWnd::ExportData TMainWnd::getExportData()
  ...
  QStringList fields;
  QString caption;
  if (pageTab==TMainWnd::SheduleTab)
    {
  ...
  return TMainWnd::ExportData(dataModel,caption,headers,fields);
  }
...
void TMainWnd::exportToCsv()
  ...
  TSettings settings;
  exportPath=settings.getXmlValue("main_window/export_path","",exportPath).toString()+"/"+exportData.caption;
...
void TMainWnd::exportToXml()
  ...
  TSettings xmlSettings(fileName,exportData.caption);
...
void TMainWnd::makeSettings()
  ...
  quint16 deadline=settings.getXmlValue("tasks_dock/reminder_deadline","",3).toUInt();
  if (deadline!=m_remindersDelegate->deadline())
    m_remindersDelegate->setReminderDeadline(deadline);
  }
...
void TMainWnd::makeArchive()
  ...
  quint16 pastDays=settings.getXmlValue("archiving/keep_past","",99).toUInt();

  // определить дату, старше которой надо удалить записи из appts
  dateTime=dateTime.addDays(-1*pastDays);
  // удалить из appts все записи старше pastDays
  QSqlQuery query(m_dataModule->database());
  query.exec(QString("delete from appts where appt_date<'%1'").arg(dateTime.toString("yyyy-MM-dd")));
...
void TMainWnd::processAuthorization()
  {
  TLoginDlg *loginDlg=new TLoginDlg(this);
...
void TMainWnd::loadDictionaries()
  ...
  // специалиазции врачей по направлениям (оно же цели посещения для таблицы посещений)
