...
bool TDataModule::openBase(const QString &baseName)
  ...
  if (m_tasksModel==nullptr)
    m_tasksModel=new TTasksModel(this,m_petcoreBase);
  m_tableHash["tasks"]=m_tasksModel;
...
qint32 TDataModule::lastAutoInc(const QString &table)
  {
  if (m_tableHash.contains(table)==false)
    return -1;

  // проверить еще наличие автоинкрементного поля
  bool hasAutoInc=false;
  QSqlRecord record=m_petcoreBase.record(table);
  for (quint16 i=0;i<record.count();i++)
    {
    if (record.field(i).isAutoValue()==false)
      continue;
  ...
  query.prepare(QString("select seq from sqlite_sequence where name='%1'").arg(table));
