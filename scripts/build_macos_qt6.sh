...
# set custom QTDIR if needed (QTDIR=/opt/Qt/6.7.3/gcc_64)
...
export lr_install_path=$lr_deploy_path/build/6.7.3/macx/release/lib
...
# TODO: собрать LimeReport
mkdir -p $lr_build_path
cd $lr_path
$qmake_command CONFIG+=release QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" BINARY_RESULT_DIR=$lr_deploy_path limereport.pro -o $lr_build_path/Makefile
[ $? -eq 0 ] || exit $?
...
# Собрать PetCore
cd $petcore_path
$qmake_command CONFIG+=release QMAKE_APPLE_DEVICE_ARCHS="x86_64 arm64" PetCore.pro -o $petcore_build_path/Makefile
[ $? -eq 0 ] || exit $?
...
#                             -I 80 -i $petcore_path/res/petcore_1.icns
#															-s "520:360" -c 400:155:130:160 
...
#cp -f $petcore_path/vers $petcore_install_path/macos-$petcore_arch-version
